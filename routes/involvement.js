...
router.post('/', [
  ...
  body('mezzo_trasporto').optional({ nullable: true }).isLength({ max: 512 }),
], async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() });
  const { event_id, quota_owner_id, participants, duration, pranzo, mezzo_trasporto } = req.body;
  if (!pool) {
    ...
    const newSub = { id: nextId(involvementSubscriptions), event_id, quota_owner_id, participants, duration: duration ?? null, pranzo: pranzo ?? null, mezzo_trasporto: mezzo_trasporto ?? null };
    involvementSubscriptions.push(newSub);
    return res.status(201).json(newSub);
  }
  try {
    const { rows } = await pool.query(
      'INSERT INTO involvement_subscriptions (event_id, quota_owner_id, participants, duration, pranzo, mezzo_trasporto) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *',
      [event_id, quota_owner_id, JSON.stringify(participants), duration ?? null, pranzo ?? null, mezzo_trasporto ?? null]
    );
